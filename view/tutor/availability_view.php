                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex flex-col md:flex-row gap-2 justify-end">
                                    <a href="<?= BASE_URL ?>index.php?url=RouteController/editAvailability
&cod_tutor=<?= $disp['cod_tutor'] ?>&hora_i=<?= $disp['hora_i'] ?>&hora_fn=<?= $disp['hora_fn'] ?>&id_horario=<?= $disp['id_horario'] ?>"
                                       class="text-purple-600 hover:text-purple-900">
                                        <i class="fas fa-edit"></i> Editar
                                    </a>
                                    <form method="POST" action="<?= BASE_URL ?>index.php?url=RouteController/deleteAvailability"
                                          onsubmit="return confirm('¿Estás seguro de eliminar esta disponibilidad?')">
                                        <input type="hidden" name="cod_tutor" value="<?= $disp['cod_tutor'] ?>">
                                        <input type="hidden" name="hora_i" value="<?= $disp['hora_i'] ?>">
                                        <input type="hidden" name="hora_fn" value="<?= $disp['hora_fn'] ?>">
                                        <input type="hidden" name="id_horario" value="<?= $disp['id_horario'] ?>">
