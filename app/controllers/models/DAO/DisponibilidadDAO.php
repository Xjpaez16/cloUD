...
class DisponibilidadDAO {
    ...
    public function getDisponibilidadByTutor($cod_tutor, $fecha = null) {
        try {
    ...
    public function create($cod_tutor, $hora_i, $hora_fn, $fecha, $id_horario, $cod_estado) {
        try {
            $sql = "INSERT INTO disponibilidad (cod_tutor, hora_i, hora_fn, fecha, id_horario, cod_estado)
                VALUES (?, ?, ?, ?, ?, ?)";
            $stmt = $this->conn->prepare($sql);
            $stmt->bind_param('isssii', $cod_tutor, $hora_i, $hora_fn, $fecha, $id_horario, $cod_estado);
            return $stmt->execute();
        } catch (Exception $e) {
